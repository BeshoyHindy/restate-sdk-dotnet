...
/// <summary>
///     A mock <see cref="SharedWorkflowContext" /> for unit testing shared workflow handlers.
///     Provides read-only in-memory state and durable promise operations (peek/resolve/reject).
///     Side effects execute inline. Calls and sends are recorded.
/// </summary>
public sealed class MockSharedWorkflowContext : SharedWorkflowContext
{
    ...
    /// <summary>Creates a new mock shared workflow context with the given key.</summary>
    public MockSharedWorkflowContext(string key = "test-key", string? invocationId = null)
    {
    ...
    /// <summary>Pre-resolves a promise so that <see cref="PeekPromise{T}" /> returns the value.</summary>
