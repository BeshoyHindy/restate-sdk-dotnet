...
public abstract class Context : IContext
{
    ...
    ///     <paramref name="code" /> (G29 — shared-core <c>sys_complete_signal</c> failure carries any
    ///     <c>TerminalFailure{code}</c>). The default body drops the code and delegates to
    ///     <see cref="SendSignalFailure(string, string, string)" /> so existing context implementations
    ///     keep working unchanged; the durable runtime context overrides this to emit the chosen code.
    /// </summary>
    public virtual ValueTask SendSignalFailure(string targetInvocationId, string name, string reason, ushort code) =>
